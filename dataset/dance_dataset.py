...
from .py3dtojittor import (axis_angle_to_quaternion,
                                  quaternion_multiply,
...
class AISTPPDataset(Dataset):
    def __init__(
        ...
        self.data = {
            "pose": pose_input,
            "beta": data["beta"],
            "motionclip_features_1": data["motionclip_features"],
    ...
    def __getitem__(self, idx):
        pose_ = self.data["pose"][idx]
        wavs_ = self.data["wavs"][idx]
        beta_ = self.data["beta"][idx]
        joint_offset_ = self.data["joint_offset"][idx]
        filename_ = self.data["filenames"][idx]
        motionclip_features_ = self.data["motionclip_features_1"][idx]
        
        feature = jt.array(np.load(filename_))
        # return self.data["pose"][idx], feature, filename_, self.data["wavs"][idx]
        return pose_, feature, filename_, wavs_, beta_, joint_offset_, motionclip_features_
    ...
    def load_aistpp(self):
        ...
        for motion, feature, wav in zip(motions, features, wavs):
            ...
            all_wavs.append(wav)
            all_joint_offset.append(joint_offset)

        
        all_pos = jt.array(all_pos)  # N x seq x 3
        all_q = jt.array(all_q)  # N x seq x (joint * 3)
        all_beta = np.array(all_beta)
        all_joint_offset = np.array(all_joint_offset)
        # downsample the motions to the data fps
        print("all_pos.shape",all_pos.shape)
        all_pos = all_pos[:, slice(None, None, self.data_stride), :]
        all_q = all_q[:, slice(None, None, self.data_stride), :]
        all_beta = all_beta[:, slice(None, None, self.data_stride), :]
        all_joint_offset = all_joint_offset[:, slice(None, None, self.data_stride), :]
        data = {"pos": all_pos, "q": all_q, "beta": all_beta, "joint_offset": all_joint_offset, "filenames": all_names, "wavs": all_wavs}
        return data
    ...
    def process_dataset(self, root_pos, local_q):
        ...
        smpl = SMPLSkeleton()
        # to Tensor
        root_pos = jt.array(root_pos)
        local_q = jt.array(local_q)
        ...
        pos_rotation= R.from_euler('x', 0, degrees=True)
        root_pos_flat = root_pos.reshape(-1, 3)     
        root_pos_rotated_flat = pos_rotation.apply(root_pos_flat)
        root_pos = root_pos_rotated_flat.reshape(*root_pos.shape)
    

        # do FK
        # positions = smpl.forward(local_q, root_pos)  # batch x sequence x 24 x 3
        positions = smpl.execute(local_q, root_pos,joint_offset)
        
        feet = positions[:, :, (7, 8, 10, 11)]
        feetv = jt.zeros(feet.shape[:3])
        feetv[:, :-1] = jt.norm(feet[:, 1:] - feet[:, :-1],dim=-1)
        contacts = (feetv < 0.01).astype(local_q.dtype) 
        ...
        assert not jt.isnan(global_pose_vec_input).any()
...
class OrderedMusicDataset(Dataset):
    ...
    def get_batch(self, batch_size, idx=None):
        ...
        return jt.array(features), seq_slice
