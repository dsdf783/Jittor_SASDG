...
def calc_physical_score(dir):
    ...
    for pkl in tqdm(it):
        ...
        root_a[:, up_dir] = jt.maximum(root_a[:, up_dir], 0) # (S-2, 3)
        # l2 norm
        # root_a = np.linalg.norm(root_a, axis=-1)  # (S-2,)
        root_a = jt.norm(root_a, axis=-1)  # (S-2,)
        scaling = root_a.max()
        root_a /= scaling
        ...
        foot_mins = jt.zeros((len(foot_v), 2))
        # foot_mins[:, 0] = np.minimum(foot_v[:, 0], foot_v[:, 1])
        # foot_mins[:, 1] = np.minimum(foot_v[:, 2], foot_v[:, 3])
        foot_mins[:, 0] = jt.minimum(foot_v[:, 0], foot_v[:, 1])
        foot_mins[:, 1] = jt.minimum(foot_v[:, 2], foot_v[:, 3])
    ...
    out = jt.mean(jt.array(scores)) * 10000
    # print(f"{dir} has a mean PFC of {out}")
    print(f"{dir} has a mean PFC of {out.item()}")
